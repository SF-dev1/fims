		...
		$('form#uid-scanship').submit(function (e) {
			...
			window.setTimeout(function () {
				...
				if (s.type == "success") {
						...
						if (typeof (index) !== "undefined") {
							...
							if (typeof (k) !== "undefined") {
								...
								if (v.item_id == s.item_id && v.scanned_qty != v.quantity && !v.scan) {
									...
									$('.item_' + v.item_id + '_' + orderItemId + ' .uids').removeClass('hide');
									success_ship[orderItemId].push(uid);
									item[k].uids.push(uid);
									item[k].scanned_qty += 1;
									$('.item_' + v.item_id + '_' + orderItemId + ' .uids .uid').text(item[k].uids.toString().replace(/,/g, ", "));

									if (item[k].scanned_qty == item[k].quantity) {
										item[k].scan = true;
										$('.item_' + v.item_id + '_' + orderItemId + ' .scanned').removeClass('hide');
									}
					...
					if (fulfillable_quantity === 0) {
						var formData = "action=save_scan_ship&scanned_items=" + JSON.stringify(success_ship);
						window.setTimeout(function () {
...
if (handler == 'fk_dashboard') {
	...
	var loadAjaxData = function ($type) {
		...
		// } else {
		$table_format = '<"row" <"col-md-4 col-sm-12" l><"col-md-8 col-sm-12 dataTables_length" <"#filters.navbar-right filter-panel"> <"#reload.navbar-right"> <"#create_report.navbar-right"> f>>rt<"table-scrollable" <"col-md-5 col-sm-12" i><"col-md-7 col-sm-12" p>>';
		// }
		...
		var oTable = table.DataTable({
			...
			"sDom": $table_format,
			"ajax": {
				url: "ajax_load.php?action=get_all_" + $type + "&token=" + new Date().getTime(),
				type: "GET",
				cache: false,
			},
		...
		$("#create_report").html('<a href="#" data-target="#generate_report" id="generate_report" role="button" class="btn btn-default generate_report" data-toggle="modal"><i class="fa fa-plus"></i></a>');
		$("#reload").html('<a href="#" class="btn btn-default reload"><i class="fa fa-sync"></i></a>');
				...
				var calendar = new FullCalendar.Calendar(calendarEl, {
					...
					eventRender: function (info) {
						var tooltip = new Tooltip(info.el, {
							...
							container: 'body',
							html: true,
							template: '<div class="tooltip fullcalendar_tooltip" role="tooltip"><div class="tooltip-arrow"></div><div class="tooltip-inner"></div></div>',
						});
					},
					nowIndicator: true,
					aspectRatio: 1.5,
	...
	var generate_reports_select = function () {
		...
		var reports = { "Invoices": ["Commission Invoice", "Commission Invoice Transaction Details"], "Payment Reports": ["Settled Transactions"], "Tax Reports": ["GSTR return report", "Sales Report", "TDS"] };

		var report_type_option = "<option></option>";
		$.each(reports, function (report_type, report) {
			report_type_option += "<option value='" + report_type + "'>" + report_type + "</option>";
		});
		...
		$('.report_type').change(function () {
			...
			if (this.value != "") {
				...
				$.each(report_sub_type, function (index, sub_report) {
					report_sub_type_option += "<option value='" + sub_report + "'>" + sub_report + "</option>";
				});
			}
	...
	var request_reports = function () {
		...
		form1.validate({
			...
			submitHandler: function (form) {
				...
				var report_sub_type = $('#report_sub_type option:selected', form1).val();
	...
	var get_report_status = function () {
		$(".update").off('click').on('click', function (e) { // off on to restrict multiple occurence of events registration
			$return = "";
			$this = $(this);
			$this.attr('disabled', true);
			...
			$.ajax({
				url: "ajax_load.php?token=" + new Date().getTime() + "&action=get_report_status&reportId=" + $reportId + "&accountId=" + $accountId + "&reportType=" + $reportType + "&reportSubType=" + $reportSubType,
				cache: false,
				type: 'POST',
				processData: false,
				async: true,
				success: function (s) {
					...
					if (s.type == "success") {
						...
						if ($reportType == "Invoices") {
							$this.closest("td").html('<a class="download btn btn-default btn-xs purple" data-reportid="' + $reportId + '" data-accountid="' + $accountId + '" data-reporttype="' + $reportType + '" data-reportsubtype="' + $reportSubType + '" data-reportfileformat="' + s.reportFileFormat + '"><i class=""></i> Download</a>');
						} else {
							$this.closest("td").html('<a class="process btn btn-default btn-xs purple" data-reportid="' + $reportId + '" data-accountid="' + $accountId + '"><i class=""></i> Process</a>');
						}
					}
				...
				error: function () {
					UIToastr.init('error', 'Report Status Update', 'Error refreshing status details!! Please retry later.');
				}
			});
			return $return;
		});
	...
	var process_report = function () {
		$(".process").off('click').on('click', function (e) { // off on to restrict multiple occurence of events registration
			$return = "";
			$this = $(this);
			$this.attr('disabled', true);
			$this.find('i').addClass('fa fa-sync fa-spin');
			...
			$.ajax({
				url: "ajax_load.php?token=" + new Date().getTime() + "&action=" + $action + "&reportId=" + $reportId + "&account_id=" + $accountId + $warehouse,
				cache: false,
				type: 'POST',
				processData: false,
				async: true,
				success: function (s) {
					...
					if (s.type == "success") {
						...
						$this.closest("td").html('<a class="process btn btn-default btn-xs purple" data-reportid="' + $reportId + '"" data-accountid="' + $accountId + '" data-reporttype="' + $reportType + '" data-reportsubtype="' + $reportSubType + '"><i class=""></i>Re-Process</a>');
						s.message = 'Total: ' + s.total + '<br/>Success: ' + s.success + '<br/>Existing: ' + s.existing + '<br/>Updated: ' + s.updated + '<br/>Error: ' + s.error;
					}
					$this.find('i').removeClass('fa fa-sync fa-spin');
					$this.attr('disabled', false);
					$return = s;
					UIToastr.init(s.type, 'Import ' + $reportType + ' Report Update', s.message);
				},
				error: function () {
					UIToastr.init('error', 'Import ' + $reportType + ' Report Update', 'Error importing payment details!! Please retry later.');
				}
			});
			return $return;
		});

		$(".download").off('click').on('click', function (e) { // off on to restrict multiple occurence of events registration
			$return = "";
			$this = $(this);
			$this.attr('disabled', true);
			$this.find('i').addClass('fa fa-sync fa-spin');
			...
			$.ajax({
				url: "ajax_load.php?token=" + new Date().getTime() + "&action=" + $action + "&reportId=" + $reportId + "&account_id=" + $accountId + "&reportType=" + $reportType + "&reportFileFormat=" + $reportFileFormat,
				cache: false,
				type: 'GET',
				processData: false,
				async: true,
				success: function (s) {
					s = $.parseJSON(s);
					if (s.type == "success") {
						$this.closest("td").html('<a class="download btn btn-default btn-xs purple" data-reportid="' + $reportId + '" data-accountid="' + $accountId + '" data-reporttype="' + $reportType + '" data-reportsubtype="' + $reportSubType + '" data-reportfileformat="' + $reportFileFormat + '"><i class=""></i> Download</a>');
	...
	var promotion_opt_in = function () {
		$('body').off('click', '.promotion_opt_in').on('click', '.promotion_opt_in', function (e) { // off on to restrict multiple occurence of events registration
			e.preventDefault();
			...
			if ($isMPInc == true) {
				...
				$(".confirm_inc_amount").off('click').on('click', function (e) { // off on to restrict multiple occurence of events registration
					...
					$preSaleStarts = moment($('#pre_sale_starts').val(), 'lll').format('X');

					$mp_validation = true;
					$date_validation = true;

					if ($preSaleStarts == "") {
						$('#pre_sale_starts').closest('.form-group').addClass('has-error');
					...
					if ($mp_validation && $date_validation) {
						window.setTimeout(function () {
							...
							$btn.attr('disabled', true);
							send_promotion_optin_request('promotion_opt_in', $accountId, $offerId, $startDate, $endDate, $isMPInc, $mpIncAmt, $preSaleStarts, $offerType, $offerValue, $offerValueType, $entityType, $this, false);
				...
				window.setTimeout(function () {
					send_promotion_optin_request('promotion_opt_in', $accountId, $offerId, $startDate, $endDate, $isMPInc, $mpIncAmt, $preSaleStarts, $offerType, $offerValue, $offerValueType, $entityType, $this, true);
				}, 500);
			}
		});
	};
	...
	var promotion_opt_in_manual = function () {
		$('body').off('click', '.promotion_opt_in_manual').on('click', '.promotion_opt_in_manual', function (e) { // off on to restrict multiple occurence of events registration
			e.preventDefault();
			...
			if ($isManual == true) {
				...
				$(".submit_manual_optin").off('click').on('click', function (e) { // off on to restrict multiple occurence of events registration
					e.preventDefault();

					$this1 = $(this);

					$('#optin_csv').closest('.form-group').removeClass('has-error');
					$('#manual_mp_inc_amount, #manual_pre_sale_starts').closest('.form-group').removeClass('has-error');

					$optin_csv = $('#optin_csv')[0].files[0];
					$mpIncAmt = $('#manual_mp_inc_amount').val();
					$preSaleStarts = moment($('#manual_pre_sale_starts').val(), 'lll').format('X');
					...
					if (typeof $preSaleStarts === 'undefined' || $preSaleStarts == "") {
						$('#pre_sale_starts').closest('.form-group').addClass('has-error');
						$date_validation = false;
					}

					if ($isMPInc && $mpIncAmt == "") {
						$('#manual_mp_inc_amount').closest('.form-group').addClass('has-error');
						$mp_validation = false;
					}
					...
					if ($file_validation && $mp_validation && $date_validation) {
						window.setTimeout(function () {
							...
							$('#manual_mp_inc_amount').closest('.form-group').removeClass('has-error');
							$('#optin_csv').closest('.form-group').removeClass('has-error');
							$('#pre_sale_starts').closest('.form-group').removeClass('has-error');
							send_promotion_optin_request('promotion_opt_in_manual', $accountId, $offerId, $startDate, $endDate, $isMPInc, $mpIncAmt, $preSaleStarts, $offerType, $offerValue, $offerValueType, $entityType, $this, false, "", "multipart/form-data");
						}, 100);
					}
				...
				$(".confirm_inc_amount").off('click').on('click', function (e) { // off on to restrict multiple occurence of events registration
					...
					if ($mpIncAmt == "") {
						...
						window.setTimeout(function () {
							$btn.find('i').addClass('fa fa-sync fa-spin');
							$btn.attr('disabled', true);
							send_promotion_optin_request('promotion_opt_in', $accountId, $offerId, $startDate, $endDate, $isMPInc, $mpIncAmt, $preSaleStarts, $offerType, $offerValue, $offerValueType, $this, false);
							$('#mp_inc_amount').closest('.form-group').removeClass('has-error');
						}, 500);
					}
					$btn.attr('disabled', false);
				...
				window.setTimeout(function () {
					send_promotion_optin_request('promotion_opt_in', $accountId, $offerId, $startDate, $endDate, $isMPInc, $mpIncAmt, $preSaleStarts, $offerType, $offerValue, $offerValueType, $this, true);
				}, 500);
			}
		});
	};
	...
	var promotion_opt_out = function () {
		$('body').off('click', '.promotion_opt_out').on('click', '.promotion_opt_out', function (e) { // off on to restrict multiple occurence of events registration
			e.preventDefault();
			...
			$.ajax({
				url: "ajax_load.php?token=" + new Date().getTime() + "&action=promotion_opt_out&accountId=" + $accountId + "&offerId=" + $offerId,
				cache: false,
				type: 'POST',
				processData: false,
				async: true,
				success: function (s) {
					...
					if (s.type == 'success') {
						...
						UIToastr.init('error', 'Promotions Opt out', 'Error Processing your Request!! ' + s.message);
					}
					$this.find('i').removeClass('fa fa-sync fa-spin');
					$this.attr('disabled', false);
				...
				error: function () {
					UIToastr.init('error', 'Promotions Opt out', 'Error Processing your Request. Please try again later!!! ' + s.message);
				}
			});
			return $return;
		});
	...
	var promotion_re_opt_in = function () {
		$('body').off('click', '.promotion_re_opt_in').on('click', '.promotion_re_opt_in', function (e) { // off on to restrict multiple occurence of events registration
			e.preventDefault();
			...
			if ($isMPInc == true) {
				...
				$(".confirm_inc_amount").off('click').on('click', function (e) { // off on to restrict multiple occurence of events registration
					...
					if ($mp_validation && $date_validation) {
						window.setTimeout(function () {
							$btn.find('i').addClass('fa fa-sync fa-spin');
							$btn.attr('disabled', true);
							send_promotion_optin_request('promotion_update', $accountId, $offerId, $startDate, $endDate, $isMPInc, $mpIncAmt, $preSaleStarts, $offerType, $offerValue, $offerValueType, $entityType, $this, false, 'ADD');
							$('#mp_inc_amount').closest('.form-group').removeClass('has-error');
						}, 500);
					}
					$btn.attr('disabled', false);
				...
				window.setTimeout(function () {
					send_promotion_optin_request('promotion_update', $accountId, $offerId, $startDate, $endDate, $isMPInc, $mpIncAmt, $preSaleStarts, $offerType, $offerValue, $offerValueType, $entityType, $this, true, 'ADD');
				}, 500);
			}
		});
	};
	...
	var promotion_re_opt_in_manual = function () {
		...
		$('body').off('click', '.promotion_re_opt_in_manual').on('click', '.promotion_re_opt_in_manual', function (e) { // off on to restrict multiple occurence of events registration
			...
			if ($isManual == true) {
				...
				$(".submit_manual_optin").off('click').on('click', function (e) { // off on to restrict multiple occurence of events registration
					...
					if ($file_validation && $mp_validation) {
						...
						window.setTimeout(function () {
							...
							$('#manual_mp_inc_amount').closest('.form-group').removeClass('has-error');
							$('#optin_csv').closest('.form-group').removeClass('has-error');
							send_promotion_optin_request('promotion_update_manual', $accountId, $offerId, $startDate, $endDate, $isMPInc, $mpIncAmt, $preSaleStarts, $offerType, $offerValue, $offerValueType, $entityType, $this, false, 'ADD', "multipart/form-data");
						}, 100);
					}
					$(this).attr('disabled', false);
					$(this).find('i').removeClass('fa fa-sync fa-spin');
				...
				window.setTimeout(function () {
					send_promotion_optin_request('promotion_update_manual', $accountId, $offerId, $startDate, $endDate, $isMPInc, $mpIncAmt, $preSaleStarts, $offerType, $offerValue, $offerValueType, $this, true, 'ADD');
				}, 500);
			}
		});
	...
	var send_promotion_optin_request = function ($action, $accountId, $offerId, $startDate, $endDate, $isMPInc, $mpIncAmt, $preSaleStarts, $offerType, $offerValue, $offerValueType, $entityType, $this, async, $updateType = "", $mime = "") {
		var formData = "";
			...
			$.ajax({
				...
				success: function (s) {
					...
					if (s.type == 'success') {
						...
						UIToastr.init(s.type, s.title, 'Error Processing your Request!! ' + s.message);
					}
					$this.find('i').removeClass('fa fa-sync fa-spin');
					$this.attr('disabled', false);
					if ($action == 'promotion_opt_in' || $action == 'promotion_opt_in_manual' || $action == 'promotion_update_manual') {
						$this.removeClass('promotion_opt_in btn-default').addClass('promotion_opt_out btn-warning');
						$this.text('Opt Out');
						$tr = $('tr').find("[data-resource-id='" + $offerId + ":" + $accountId + "']");
						$tr.find('a').css({ 'background-color': 'lightgreen', 'border-color': 'lightgreen' });
						if (s.type == "success" && ($action == "promotion_opt_in_manual" || $action == "promotion_opt_in" || $action == 'promotion_update_manual')) {
							$('#confirm_mp_inc_amount').modal('hide');
							$('#manual_optin').modal('hide');
							$('#mp_inc_amount').val("");
							$(".form-horizontal").find('.btn-submit i').removeClass('fa fa-sync fa-spin');
				...
				error: function () {
					UIToastr.init('error', 'Promotions Optin Request', 'Error Processing your Request. Please try again later!!! ' + s.message);
				}
			});
		}, 500);
	};
	...
	var get_promotion_lid = function () {
		$("body").off('click', '.get_promotion_lid').on('click', '.get_promotion_lid', function () { // off on to restrict multiple occurence of events registration
			$this = $(this);
			$accountId = $this.data('accountid');
			$offerId = $this.data('offerid');
			$entityType = $this.data('entitytype');
			...
			$.ajax({
				...
				success: function (s) {
					...
					if (s.type == 'success') {
						...
						UIToastr.init('error', 'Get Eligible Listings', 'Error Processing your Request!! ' + s.message);
					}
					$this.attr('disabled', false);
					$this.removeClass('get_promotion_lid').addClass('get_promotion_lid');
					if ($opted) {
						$this.html('<i class="fa fa-download"></i> Download Opted Listings</a>&nbsp;');
					} else {
						$this.html('<i class="fa fa-download"></i> Download Listings</a>&nbsp;');
					}
					$this.attr('href', s.file_path);
					$this.attr('target', '_blank');
				},
				error: function () {
					UIToastr.init('error', 'Get Eligible Listings', 'Error Processing your Request. Please try again later!!! ' + s.message);
				}
			});
		});
	};
	...
	var wrongSKU_handleTable = function () {
		...
		function editRow(oTable, nRow) {
			...
			jqTds[1].innerHTML = '<input type="text" class="form-control input-large" value="' + aData[1] + '">';
			// jqTds[2].innerHTML = aData[2];
			// jqTds[3].innerHTML = aData[3];
			// jqTds[4].innerHTML = aData[4];
			jqTds[5].innerHTML = '<a class="edit btn btn-default btn-xs purple" href=""><i class="fa fa-check"></i> Save</a>';
			jqTds[6].innerHTML = '<a class="cancel btn btn-default btn-xs purple" href=""><i class="fa fa-times"></i> Cancel</a>';
		...
		function saveRow(oTable, nRow) {
			...
			oTable.fnUpdate('<a class="edit btn btn-default btn-xs purple" href=""><i class="fa fa-edit"></i> Edit</a>', nRow, 5, false);
			oTable.fnUpdate('<a class="delete btn btn-default btn-xs purple" href=""><i class="fa fa-trash-o"></i> Delete</a>', nRow, 6, false);
			...
			$.ajax({
				...
				success: function (s) {
					...
					if (s.type == 'success') {
						...
						UIToastr.init('success', 'Update Wrong SKU', 'Error Processing your Request!! ' + s.message);
					}
				},
				error: function () {
					UIToastr.init('error', 'Update Wrong SKU', 'Error Processing your Request. Please try again later!!!');
				}
			});
		...
		function cancelEditRow(oTable, nRow) {
			...
			oTable.fnUpdate('<a class="edit btn btn-default btn-xs purple" href="">Edit</a>', nRow, 4, false);
			oTable.fnDraw();
		}
				...
				$.ajax({
					...
					success: function (s) {
						...
						if (s.type == 'success') {
							...
							error1.text('Error Processing your Request!! ' + s.message).show();
							UIToastr.init('success', 'Delete Wrong SKU', 'Error Processing your Request!! ' + s.message);
						}
					},
					error: function () {
						UIToastr.init('error', 'Delete Wrong SKU', 'Error Processing your Request. Please try again later!!!');
					}
				});
				oTable.api().ajax.reload();
				return;
		...
		table.on('click', '.edit', function (e) {
			...
			if (nEditing !== null && nEditing != nRow) {
				...
				editRow(oTable, nRow);
				nEditing = nRow;
			} else if (nEditing == nRow && this.innerHTML == '<i class="fa fa-check"></i> Save') {
				...
				$.ajax({
					...
					success: function (s) {
						...
						if (s.type == 'success') {
							...
							error1.text('Error Processing your Request!! ' + s.message).show();
							UIToastr.init('error', 'Add Wrong SKU', 'Error Processing your Request!! ' + s.message);
						}
					},
					error: function () {
						UIToastr.init('error', 'Add Wrong SKU', 'Error Processing your Request. Please try again later!!!');
					}
				});
	...
	jQuery(document).ready(function ($) {
		...
		$("#portlet_new a, #portlet_packing a, #portlet_rtd a").click(function (e) {
			...
			if (($type != 'packlist' && $type != 'packlist-fbf') && shipment_ids.length == 0) {
				...
				currentRequest = $.ajax({
					...
					success: function (s) {
						...
						if (s.success != 0) {
							...
							$('.alert-success').append('Moved ' + s.success + ' orders to Pack.');
						}
						if (s.error != 0) {
				...
				currentRequest = $.ajax({
					...
					success: function (s) {
						...
						if (s.success != 0) {
							...
							UIToastr.init('success', 'Label Generation', 'Successfully generated labels for ' + s.success + ' orders.');
							...
							currentRequest = $.ajax({
								...
								cache: false,
								type: 'POST',
								data: "action=mark_to_pack&shipment_ids=" + s.success_shipment_ids,
								success: function (s) {
									s = $.parseJSON(s);
									...
									if (s.success != 0) {
										...
										UIToastr.init('success', 'Mark to Pack', 'Moved ' + s.success + ' orders to Pack.');
									}
									if (s.error != 0) {
										// $('.alert-danger').show().delay(10000).fadeOut('slow');
										// $('.alert-danger').append('Unable to update '+s.error+' orders.');
										UIToastr.init('success', 'Mark to Pack', 'Unable to update ' + s.error + ' orders.');
									}
									clearInterval(interval);
									NProgress.done(true);
								},
								error: function () {
									clearInterval(interval);
									NProgress.done(true);
									UIToastr.init('error', 'Mark to Pack', 'Error Processing your Request! <br />Please try again.');
									// alert('Error Processing your Request!!');
								}
							});
						...
						if (s.error != 0) {
							UIToastr.init('error', 'Label Generation', 'Unable to generate labels for ' + s.error + ' orders.' + s.error_details);
						}
						// update_checked_c(tab);
						clearInterval(interval);
						NProgress.done(true);
					...
					error: function () {
						...
						NProgress.done(true);
						console.log('Error Processing your Request!!');
						UIToastr.init('error', 'Label Generation', 'Error Processing your Request! <br />Please try again.');
					}
				});
			...
			} else if ($type == "label" || $type == "invoice" || $type == "form" || $type == "label-invoice") {
		...
		// MARK RTD
		$("#mark_rtd").on("show.bs.modal", function (e) {
		...
		$("#mark-rtd").submit(function (e) {
			...
			$.ajax({
				url: "ajax_load.php?token=" + new Date().getTime(),
				type: 'POST',
				data: "action=mark_rtd&trackin_id=" + $trackin_id + "&account=" + $account,
				success: function (s) {
					var as = $.parseJSON(s);
					$('.list-container').show();
					$.each(as, function (k, s) {
						if (s.type == 'success') {
							...
							count_s = count_s + 1;
							$('.scan-passed').html(count_s + '<span class="scan-passed-ok"><i class="icon icon-check-circle" aria-hidden="true"></i></span>');
							...
							count_e = count_e + 1;
							$('.scan-failed-count').html(count_e + '<span class="cancel-icon"><i class="icon icon-remove-sign" aria-hidden="true"></i></span>');
							if (rtd_trackingIds.indexOf($trackin_id) === -1) {
								rtd_trackingIds.push($trackin_id);
							}
						}
		...
		// MARK SHIPPED
		$("#mark_shipped").on("show.bs.modal", function (e) {
			$("body").addClass("modal-open");
			// RESET
			s_count_s = 0;
			s_count_e = 0;
		...
		// MARK CANCELLED
		$("#mark_cancelled").on("show.bs.modal", function (e) {
			$("body").addClass("modal-open");
			// RESET
			c_count_s = 0;
			c_count_e = 0;
		...
		$("#mark-cancelled").submit(function (e) {
			...
			$.ajax({
				url: "ajax_load.php?token=" + new Date().getTime(),
				type: 'POST',
				data: "action=mark_cancel&trackin_id=" + $trackin_id + "&account=" + $account,
				success: function (s) {
					s = $.parseJSON(s);
					$('.list-container').show();
					if (s.type == 'success') {
						...
						$('.scan-passed').html(c_count_s + '<span class="scan-passed-ok"><i class="icon icon-check-circle" aria-hidden="true"></i></span>');
						...
						$('.scan-failed-count').html(c_count_e + '<span class="cancel-icon"><i class="icon icon-remove-sign" aria-hidden="true"></i></span>');
						if (cancel_trackingIds.indexOf($trackin_id) === -1) {
							cancel_trackingIds.push($trackin_id);
						}
					}
		...
		$("#mark_return_received").on("show.bs.modal", function (e) {
			$("body").addClass("modal-open");
			// RESET
			r_count_s = 0;
			r_count_e = 0;
				...
				success: function (s) {
					...
					$.each(as, function (k, s) {
						if (s.type == 'success') {
							...
							$('.scan-passed').html(r_count_s + '<span class="scan-passed-ok"><i class="icon icon-check-circle" aria-hidden="true"></i></span>');
							...
							$('.scan-failed-count').html(r_count_e + '<span class="cancel-icon"><i class="icon icon-remove-sign" aria-hidden="true"></i></span>');
							if (return_trackingIds.indexOf($trackin_id) === -1) {
								return_trackingIds.push($trackin_id);
							}
						}
		...
		// MARK ACKNOWLEDGE
		$("#mark_acknowledge_return").on("show.bs.modal", function (e) {
			$("body").addClass("modal-open");
			// RESET
			r_count_s = 0;
			r_count_e = 0;
		...
		// SEARCH & CLAIM
		$("#search_claim_return").on("show.bs.modal", function (e) {
			$("body").addClass("modal-open");
			...
			$.ajax({
				...
				success: function (s) {
					...
					if (s.type == "error") {
						...
						$("#search_claim_return .modal-body-nothing-found").text(s.message).removeClass('hide');
						...
						s = s.order;
						$("#search_claim_return .product-image img").attr('src', s.productImage);
						...
						$("#search_claim_return .customer_comment").text(s.r_comments === "" ? "No comments from buyer" : s.r_comments);
		...
		$("#search-orders").submit(function (e) {
			...
			$.ajax({
				...
				success: function (s) {
					s = $.parseJSON(s);
					if (s.type == 'error') {
						$("#search_order .alert-danger").text("No order details found.").show();
					} else {
						if (s.content != "") {
							...
							$(".modal-body-details").html('<div class="modal-body-empty text-center">No order found!!!</div>');
						}
	...
	function spf_claim_order() {
		...
		$("#claim-spf .product_uids .checkboxes").off().on('change', function () {
			var checked = $(this).prop('checked');
			group = $(this).val();

			$('.fieldset_' + group).addClass('hide');
			$('.product_condition_list_group_' + group + ' .radio').attr('disabled', true);
			if (checked) {
		...
		$("#claim-spf .product_condition_list .radio").off().on('change', function () {
			group = $(this).data('group');
			product_condition = $(this).val();
			$(".issue_product_list_group_" + group + ", .product_rto_claim_details").addClass('hide');
			$(".issue_product_list_group_" + group + " .checkboxes, .issue_product_list_group_" + group + " .radio").attr('disabled', true).attr('required', false);
	...
	function reset_spf_modal_inputs() {
		$("#claim-spf .product_claim_details, #claim-spf .claim_images, #claim-spf .product_rto_claim_details").addClass('hide');
		...
		var returns_pod = function () {
			...
			form1.validate({
				...
				submitHandler: function (form) {
					...
					window.open("ajax_load.php?token=" + new Date().getTime() + query, '_blank');
	...
	function loadajaxOrders(order_type, location) {
		...
		currentRequest = typeof currentRequest !== 'undefined' ? currentRequest : null;
		$(".order_content").empty();
		// add loader icon
		$(location + " tbody").html("<tr><td colspan='3'><center><i class='fa fa-sync fa-spin'></i></center></td></tr>");
		...
		currentRequest = $.ajax({
			...
			success: function (s) {
				...
				$("#mark-acknowledge-return-single [name='claim']").bind('change', function () {
					if ($(this).val() == 'yes' || $(this).val() == 're-claim') {
						$(".claim_id").removeClass('hide');
						$("#mark-acknowledge-return-single input[name=claim_id]").rules("add", {
							minlength: 21,
							maxlength: 21,
							required: true
						});
						...
						$("#mark-acknowledge-return-single input[name=claim_id]").rules("remove");
					}
				});
		...
		$.ajax({
			...
			success: function (s) {
				...
				if (handler == "order") {
					...
					$(".portlet_in_transit.count").text("(" + (arr.orders.in_transit) + ")");
					$(".portlet_out_for_delivery.count").text("(" + (arr.orders.out_for_delivery) + ")");
					$(".portlet_delivered.count").text("(" + (arr.orders.delivered) + ")");
					$(".portlet_received.count").text("(" + (arr.orders.return_received) + ")");
					$(".portlet_claimed.count").text("(" + (arr.orders.return_claimed) + ")");
					$(".portlet_claimed_undelivered.count").text("(" + (arr.orders.return_claimed_undelivered) + ")");
					$(".portlet_return_completed.count").text("(" + (arr.orders.return_completed) + ")");
					$(".portlet_return_unexpected.count").text("(" + (arr.orders.return_unexpected) + ")");
				}
				var dropdowns = "";
				if (arr.logistic == "") {
					dropdowns += '<li class="list-group-item"><a><center>No Orders Yet!</center></a></li>';
				} else {
					for (var account in arr.logistic) {
						...
						for (var partners in arr.logistic[account]) {
							...
							for (var type in arr.logistic[account][partners]) {
								final_dropdowns += '<li class="list-group-item d-flex justify-content-between align-items-center"><a>' + type + '<span class="badge badge-default badge-pill">' + arr.logistic[account][partners][type] + '</span></a></li>';
								count += arr.logistic[account][partners][type];
								lp_count += arr.logistic[account][partners][type];
							}
							inner_dropdowns += '<li class="list-group-item group-sub-header"><a>' + partners + '<span class="badge badge-primary badge-pill">' + lp_count + '</span></a></li>' + final_dropdowns;
						}
						dropdowns += '<li class="list-group-item group-header"><a>' + account + '<span class="badge badge-primary badge-pill">' + count + '</span></a></li>' + inner_dropdowns;
					}
				}
				$('.logistic_details').html('<div class="btn-group btn-group-solid"><button type="button" class="btn btn-info dropdown-toggle" data-toggle="dropdown"><i class="fa fa-ellipsis-horizontal"></i> Pickup Details <i class="fa fa-angle-down"></i></button><ul class="dropdown-menu list-group">' + dropdowns + '</ul></div>');
				...
				if (arr.dispatchByDates != "" && arr.dispatchByDates != null) {
					...
					$.each(arr.dispatchByDates.FBF_LITE, function (k, v) {
						$opt_fbf += "<li><a class='generate_upcoming_picklist' data-order_type='FBF_LITE' data-dbd='" + k + "'>" + k + " (" + v + ")</a></li>";
					});
					$.each(arr.dispatchByDates.NON_FBF, function (k, v) {
						$opt_non += "<li><a class='generate_upcoming_picklist' data-order_type='NON_FBF' data-dbd='" + k + "'>" + k + " (" + v + ")</a></li>";
					});
	...
	function get_return_delivery_breached_orders() {
		...
		$.ajax({
			...
			success: function (s) {
				...
				$("#return_delivery_breached .modal-body i").removeClass("fa fa-sync fa-spin");
			...
			error: function () {
				$("#return_delivery_breached .modal-body i").addClass("hide");
				$("#return_delivery_breached .modal-body i").removeClass("fa fa-sync fa-spin");
				$('#return_delivery_breached .modal-body').html('Error Processing your Request!!');
			}
		});
	}

	function raise_ticket_return_delivery_breached_orders() {
		$('.raise_ticket').off('click').on('click', function () {
			$this = $(this);
			...
			$mandatory_fields = '{"order_id":"' + $orderId + '","tracking_id":"' + $trackingId + '"}';
			...
			$.ajax({
				...
				type: 'POST',
				async: true,
				data: "action=create_support_ticket&account=" + $accountId + "&mandatory_fields=" + $mandatory_fields + "&issueType=" + $issueType + "&subject=" + $subject + "&content=" + $content + "&insert=false",
				success: function (s) {
					s = $.parseJSON(s);
					if (s.type == "success") {
						// update_claim_status
						$.ajax({
							...
							type: 'POST',
							async: true,
							data: "action=update_claim_status&return_id=" + $returnId + "&claim_id=" + s.incidentId + "&condition=" + $condition,
							success: function (xs) {
								// $this.attr('disabled', false);
							...
							error: function (xhr, textStatus, errorThrown) {
								...
								UIToastr.init(xs.type, 'Incident Creation', 'Error saving support ticket!! Please retry later. <br />ERROR:' + textStatus + ':' + errorThrown);
							}
						});
					}
				...
				error: function (xhr, textStatus, errorThrown) {
					...
					$this.find("i").removeClass("fa fa-sync fa-spin");
					console.log(textStatus + ':' + errorThrown);
					UIToastr.init('error', 'Incident Creation', 'Error creating support ticket!! Please retry later. <br />ERROR:' + textStatus + ':' + errorThrown);
				}
			});
		});
	}
	...
	function redownload_label() {
		$(".redownload_label").off('click').on('click', function () {
			$button = $(this);
			...
			currentReq = $.ajax({
				...
				cache: false,
				type: 'POST',
				data: "action=generate&type=redownload-label&shipment_ids=" + $shipmentId,
				beforeSend: function () {
					if (currentRequest != null) {
						currentRequest.abort();
					} else {
	...
	function generate_upcoming_picklist() {
		$('.generate_upcoming_picklist').off('click').on('click', function (e) {
			e.preventDefault();
			$order_type = $(this).data('order_type');
			$dbd = $(this).data('dbd');
			generate_packlist($order_type, $dbd);
		});
		$('.upcoming_picklist').off('click').on('click', function (e) {
			e.preventDefault();
	...
	function get_replacement_orders() {
		...
		$.ajax({
			...
			success: function (s) {
				...
				$("#order_replacements .modal-body i").removeClass("fa fa-sync fa-spin");
	...
	function get_duplicate_orders() {
		...
		$.ajax({
			...
			error: function () {
				$("#order_duplicate .modal-body i").addClass("hide");
				$("#order_duplicate .modal-body i").removeClass("fa fa-sync fa-spin");
				$('#order_duplicate .modal-body').html('Error Processing your Request!!');
			}
		});
	}
	...
	function doubted_orders_checkbox_selection($div) {
		...
		$('#' + $div + ' button[type="submit"]').click(function () {
			...
			if (allChecked.length > 0) {
				...
				$("#" + $div + " button[type='submit'] i").addClass("fa fa-sync fa-spin");
				...
				$(allChecked).each(function (index, element) {
					...
					if (typeof orderItemIds[account][group] === 'undefined') {
						...
						if (group == "multi_quantity") {
							orderItemIds[account][group][glength] = { [orderItemId]: quantity };
						} else {
				...
				$.ajax({
					...
					success: function (s) {
						...
						$("#" + $div + " button[type='submit'] i").removeClass("fa fa-sync fa-spin");

						$templateContent = "";
						$.each(s.accounts, function (i, account) {
							$.each(account, function (k, group) {
								$templateContent += "<div class='col-md-4 text-left'><div class='well'><span>" + group.html + "</span><br /><br /><span class='create_ticket_content'><button class='btn btn-success create_ticket' type='button' value='Create Ticket' data-account='" + i + "' data-orderid='" + group.orderId + "' data-orderitemids='" + group.orderItemIds + "' data-issuetype='" + group.issueType + "' data-subject='" + group.subject + "' data-content='" + group.ticket_content + "'><i></i> Create Ticket</button></span></div></div>"
							});
						});
	...
	function create_support_ticket() {
		$('.create_ticket').off('click').on('click', function () {
			$this = $(this);
			$this.attr('disabled', true);
			$this.find("i").removeClass("hide");
			$this.find("i").addClass("fa fa-sync fa-spin");
			...
			$mandatory_fields = '{"order_id":"' + $orderId + '","order_item_id":"' + $orderItemIds + '"}';
			...
			$.ajax({
				...
				type: 'POST',
				async: true,
				data: "action=create_support_ticket&account=" + $account + "&mandatory_fields=" + $mandatory_fields + "&issueType=" + $issueType + "&subject=" + $subject + "&content=" + $content,
				success: function (s) {
					s = $.parseJSON(s);
					$this.attr('disabled', false);
					$this.find("i").addClass("hide");
					$this.find("i").removeClass("fa fa-sync fa-spin");
					if (s.type == "success") {
						$this.parent().html('<div class="alert alert-success"><strong>Success!</strong> ' + s.message + '</div>');
						$orderItemIds += ",";
				...
				error: function (xhr, textStatus, errorThrown) {
					...
					$this.find("i").removeClass("fa fa-sync fa-spin");
					console.log(textStatus + ':' + errorThrown);
					UIToastr.init('error', $subject, 'Error creating support ticket!! Please retry later. <br />ERROR:' + textStatus + ':' + errorThrown);
				}
			});
	...
	function get_spf_amount() {
		$("#update-claim .get_spf").off('click').on('click', function () {
			$this = $(this);
			$this.attr("disabled", true);
			$this.find("i").addClass("fa fa-sync fa-spin");
			...
			$.ajax({
				...
				cache: false,
				type: 'GET',
				data: "action=get_spf_amount&orderItemId=" + $orderItemId + "&accountName=" + $accountName,
				success: function (s) {
					s = $.parseJSON(s);
					$this.attr("disabled", false);
					$this.find("i").removeClass("fa fa-sync fa-spin");
					...
					if (s.type == "success") {
						...
						$('.sellement_percentage').removeClass('hide').text(percent.toFixed(2) + '%');
				...
				error: function () {
					$this.attr("disabled", false);
					$this.find("i").removeClass("fa fa-sync fa-spin");
					UIToastr.init('error', 'SPF Details', 'Error fetching SPF details!! Please retry later.');
					// $('#order_replacements .modal-body').html('Error Processing your Request!!');
				}
			});
		});

		$("#update-claim .spf_reclaim").off('click').on('click', function (e) {
			e.preventDefault();
			$this = $(this);
			$this.attr('disabled', true);
			$this.find("i").removeClass("hide");
			...
			window.setTimeout(function () {
				$.ajax({
					...
					cache: false,
					type: 'POST',
					data: "action=create_support_ticket&account=" + $account + "&mandatory_fields=" + $mandatory_fields + "&issueType=" + $issueType + "&subject=" + $subject + "&content=" + $content + "&insert=false",
					success: function (s) {
						s = $.parseJSON(s);
						...
						if (s.type == "success") {
							$.ajax({
								...
								cache: false,
								type: 'POST',
								data: "action=update_claim_id&pk=" + $returnid + "&value=" + s.incidentId + "&approved_spf=" + $approved_spf,
								success: function (r) {
									r = $.parseJSON(r);
									if (r.type == "success")
										UIToastr.init(r.type, 'Update Incident ID', "Succesfully updated Incident ID");
									else
										UIToastr.init(r.type, 'Update Incident ID', r.msg);
	...
	function lost_claim() {
		...
		$('.file_lost_claim').bind('click', function () {
			...
			$id = $(this).data("id");
			$("#lost_claim .product-image img").attr('src', $("#" + $id + " .ordered-product-image img").attr('src'));
			$("#lost_claim .article-title").text($("#" + $id + " .product_name").text());
			$("#lost_claim .sku").text($("#" + $id + " .sku").text());
			$("#lost_claim .order_item_id").text($("#" + $id + " .order_item_id").text());
			...
			$("#lost_claim .sub_reason").text($("#" + $id + " .return_sub_reason").text());
			$("#lost_claim .customer_comment").text($("#" + $id + " .comments").text() === "" ? "No comments from buyer" : $("#" + $id + " .comments").text());
			...
			$($ids).each(function (id, idv) {
				$(idv).each(function (k, v) {
					$(".products_condition").append('<div class="form-group" id="product_condition_' + v.pid + '"><label class="col-md-3 control-label">Product condition for ' + v.sku + '<span class="required" aria-required="true">*</span></label><div class="col-md-9 radio-list"><label class="radio-inline"><div class="radio"><span><input class="product_condition_' + v.pid + '" name="product_condition[' + v.pid + ']" value="undelivered" type="radio" checked></span></div>Undelivered</label><div class="form_product_condition_error"></div></div></div>');
					$pid.push(v.pid);
				});
			});

			// Lets get the functionlaity back of dynmically added radio list
			$('#lost-claim .form-group').on('click', 'input[type=radio]', function () {
				$(this).closest('.form-group').find('.radio, span').removeClass('checked');
	...
	function return_acknowledge() {
		...
		$('.acknowledge_return').bind('click', function () {
			...
			$id = $(this).data("id");
			$type = $(this).data("type");
			$("#mark_acknowledge_return_single .product-image img").attr('src', $("#" + $id + " .ordered-product-image img").attr('src'));
			$("#mark_acknowledge_return_single .article-title").text($("#" + $id + " .product_name").text());
			$("#mark_acknowledge_return_single .sku").text($("#" + $id + " .sku").text());
			$("#mark_acknowledge_return_single .order_item_id").text($("#" + $id + " .order_item_id").text());
			$("#mark_acknowledge_return_single .fsn").text($("#" + $id + " .fsn").text());
			$("#mark_acknowledge_return_single .order_id").text($("#" + $id + " .order_id").text());
			$("#mark_acknowledge_return_single .order_date").text($("#" + $id + " .order_date").text());
			$("#mark_acknowledge_return_single .amount").text($("#" + $id + " .amount").text());
			$("#mark_acknowledge_return_single .qty").text($("#" + $id + " .qty").text());
			$("#mark_acknowledge_return_single .return_qty").text($("#" + $id + " .return_qty").text());
			$("#mark_acknowledge_return_single .type").text($("#" + $id + " .type").text());
			$("#mark_acknowledge_return_single .reason").text($("#" + $id + " .return_reason").text());
			$("#mark_acknowledge_return_single .sub_reason").text($("#" + $id + " .return_sub_reason").text());
			$("#mark_acknowledge_return_single .customer_comment").text($("#" + $id + " .comments").text() === "" ? "No comments from buyer" : $("#" + $id + " .comments").text());
			$("#mark_acknowledge_return_single .tracking_id").text($("#" + $id + " .tracking_id").text());
			...
			$($ids).each(function (id, idv) {
				$(idv).each(function (k, v) {
					if ($type == 'unexpected') {
						$(".products_condition").append('<div class="form-group" id="product_condition_' + v.pid + '"><label class="col-md-3 control-label">Product condition for ' + v.sku + '<span class="required" aria-required="true">*</span></label><div class="col-md-9 radio-list"><label class="radio-inline"><div class="radio"><span><input class="product_condition_' + v.pid + '" name="product_condition[' + v.pid + ']" value="missing" type="radio" checked></span></div>Missing</label><div class="form_product_condition_error"></div></div></div>').hide();
						location = 'return_unexpected';
					} else {
						$(".products_condition").append('<div class="form-group" id="product_condition_' + v.pid + '"><label class="col-md-3 control-label">Product condition for ' + v.sku + '<span class="required" aria-required="true">*</span></label><div class="col-md-9 radio-list"><label class="radio-inline"><div class="radio"><span><input class="product_condition_' + v.pid + '" name="product_condition[' + v.pid + ']" value="saleable" type="radio"></span></div>Saleable</label><label class="radio-inline"><div class="radio"><span><input class="product_condition_' + v.pid + '" name="product_condition[' + v.pid + ']" value="damaged" type="radio"></span></div>Damaged</label><label class="radio-inline"><div class="radio"><span><input class="product_condition_' + v.pid + '" name="product_condition[' + v.pid + ']" value="wrong" type="radio"></span></div>Wrong</label><div class="form_product_condition_error"></div></div></div>');
			...
			$(".products_uid").html("");
			$products_uid = '<div class="form-group"><label class="col-md-3 control-label">Product UID<span class="required" aria-required="true">*</span></label><div class="col-md-9 radio-list">';
			$uids = $("#" + $id + " .uids").text();
			if ($uids != "") {
				$uids = $.parseJSON($uids);
				$($uids).each(function (k, uid) {
					$products_uid += '<label class="radio-inline"><div class="radio"><span><input class="products_uid" name="uid" value="' + uid + '" type="radio"></span></div>' + uid + '</label>';
				});
				if ($uids.length > 1)
					$products_uid += '<label class="radio-inline"><div class="radio"><span><input class="products_uid" name="uid" value="' + $uids.join(",") + '" type="radio"></span></div>All</label>';
				$products_uid += '<div class="form_uid_error"></div></div></div>';
				$(".products_uid").append($products_uid);
			}

			// Lets get the functionlaity back of dynmically added radio list
			$('#mark-acknowledge-return-single .form-group').on('click', 'input[type=radio]', function () {
				$(this).closest('.form-group').find('.radio, span').removeClass('checked');
				$(this).closest('.radio, span ').addClass('checked');
			});
			...
			$('#mark-acknowledge-return-single [name^="product_condition').each(function () {
				$(this).rules('add', {
					required: true,
					messages: { // optional
	...
	function update_claim() {
		$('.update_claim').off('click').on('click', function () {
			$id = $(this).data("id");
			$("#update_claim .product-image img").attr('src', $("#" + $id + " .ordered-product-image img").attr('src'));
			$("#update_claim .article-title").text($("#" + $id + " .product_name").text());
			$("#update_claim .sku").text($("#" + $id + " .sku").text());
			$("#update_claim .order_item_id").text($("#" + $id + " .order_item_id").text());
			$("#update_claim .fsn").text($("#" + $id + " .fsn").text());
			$("#update_claim .return_id").val($("#" + $id + " .return_id").text());
			$("#update_claim .claim_date").val($("#" + $id + " .claim_date").text());
			$("#update_claim .claim_staus").val($("#" + $id + " .claim_staus").text());
			$("#update_claim .claim_id").val($("#" + $id + " .claim_id").text());
			$("#update_claim .product_condition").val($("#" + $id + " .product_condition").text());
			$("#update_claim .order_id").text($("#" + $id + " .order_id").text());
			$("#update_claim .order_date").text($("#" + $id + " .order_date").text());
			$("#update_claim .item_amount").text($("#" + $id + " .item_amount").text());
			$("#update_claim .shipping_amount").text($("#" + $id + " .shipping_amount").text());
			$("#update_claim .amount").text($("#" + $id + " .amount").text());
			$("#update_claim .qty").text($("#" + $id + " .qty").text());
			$("#update_claim .type").text($("#" + $id + " .type").text());
			$("#update_claim .reason").text($("#" + $id + " .return_reason").text());
			$("#update_claim .sub_reason").text($("#" + $id + " .return_sub_reason").text());
			$("#update_claim .customer_comment").text($("#" + $id + " .comments").text() === "" ? "No comments from buyer" : $("#" + $id + " .comments").text());
			$("#update_claim .tracking_id").text($("#" + $id + " .tracking_id").text());
			$("#update_claim .get_spf").attr('data-order-item-id', $("#" + $id + " .order_item_id").text());
			$("#update_claim .get_spf").attr('data-account-name', $("#" + $id + " .account_name").text());
			$("#update_claim .get_spf").attr('data-item_amount', $("#" + $id + " .item_amount").data('item_amount'));
			$("#update_claim .get_spf").attr('data-shipping_amount', $("#" + $id + " .shipping_amount").data('shipping_amount'));
			$("#update_claim .get_spf").attr('data-total_amount', $("#" + $id + " .amount").data('amount'));
			$("#update_claim .get_spf").attr('data-return_type', $("#" + $id + " .type").text());
			$("#update_claim .sellement_percentage").html("").addClass('hide');

			$("#update_claim .spf_reclaim").removeData(['base_incident', 'returnid', 'orderItemId', 'account', 'content', 'approved_spf']);
			$("#update_claim .spf_reclaim").attr('data-base_incident', $("#" + $id + " .claim_id").text());
			$("#update_claim .spf_reclaim").attr('data-returnid', $("#" + $id + " .return_id").text());
			$("#update_claim .spf_reclaim").attr('data-orderItemId', $("#" + $id + " .order_item_id").text());
			$("#update_claim .spf_reclaim").attr('data-account', $(".reclaim_details_" + $id).data('account'));
			$("#update_claim .spf_reclaim").attr('data-content', $(".reclaim_details_" + $id).data('content'));
			$("#update_claim .spf_reclaim").attr('data-approved_spf', $(".reclaim_details_" + $id).data('approved_spf'));
			...
			init: function (id) {
				...
				form.validate({
					...
					submitHandler: function (form) {
						...
						$pids = $(id + " input[name='pids']").val().split(",");
						var $condition = {};
						$($pids).each(function (k, v) {
							$condition[v] = $(id + " input[name='product_condition[" + v + "]']:checked").val();
						})
						...
						$.ajax({
							...
							cache: false,
							type: 'POST',
							data: "action=update_claim_status&return_id=" + $return_id + "&claim_id=" + $claim_id + "&condition=" + JSON.stringify($condition),
							success: function (s) {
								s = $.parseJSON(s);
				...
				form.validate({
					...
					errorPlacement: function (error, element) { // render error placement for each input type
						if (element.attr("name").indexOf("product_condition") >= 0) { // for uniform radio buttons, insert the after the given container
							...
							error.addClass("no-left-padding").insertAfter(id + " .form_uid_error");
						} else if (element.attr("name") == "claim") {
							error.addClass("no-left-padding").insertAfter(id + " .form_claim_error");
						} else {
							error.insertAfter(element); // for other inputs, just perform default behavoir
					...
					submitHandler: function (form) {
						...
						$.ajax({
							...
							cache: false,
							type: 'POST',
							data: "action=update_claim_status&return_id=" + $return_id + "&claim_id=" + $claim_id + "&condition=" + JSON.stringify($condition) + "&re_claim=" + $re_claim + "&uid=" + $uid,
							success: function (s) {
								s = $.parseJSON(s);
								$(id + ' btn-success').attr('disabled', false);
								if (s.type == 'success') {
		...
		return {
			init: function (id) {
				...
				form.validate({
					...
					success: function (label) {
						if (label.attr("for") == "claim_staus") { // for checkboxes and radio buttons, no need to show OK icon
							label
					...
					submitHandler: function (form) {
						...
						var claim_status = $('#' + form.id + " input[name='claim_staus']:checked").val();
						var claim_comments = $('#' + form.id + " textarea[name='claim_comments']").val();
						var claim_amount = $('#' + form.id + " input[name='claim_reimbursment']").val();
						var product_condition = $('#' + form.id + " input[name='product_condition']").val();

						console.log(product_condition);

						var received_on = "";
						if (product_condition == "Undelivered") {
							received_on = $('#' + form.id + " input[name='received_on']").val();
						...
						$.ajax({
							...
							cache: false,
							type: 'POST',
							data: "action=update_claim_details&return_id=" + return_id + "&claim_status=" + claim_status + "&claim_comments=" + claim_comments + "&claim_amount=" + claim_amount + received_on,
							async: true,
							success: function (s) {
								s = $.parseJSON(s);
								if (s.type == "success") {
								...
								if (s.type == 'error') {
									...
									$('.form-actions .btn-success').attr('disabled', false);
									$('.form-actions i').removeClass('icon-refresh');
									$('.re_error').text('Unable to process request. Please retry');
								}
								$('#' + id + ' btn-success').attr('disabled', false);
								UIToastr.init(s.type, 'Claim Update', s.message);
							},
			...
			init: function (spf_files) {
				...
				form.validate({
					...
					submitHandler: function (form) {
						...
						$('.form-actions .btn-success', form).attr('disabled', true).find('i').addClass('fa fa-sync fa-spin');
						$('.re_error').text("");
						...
						$.ajax({
							...
							success: function (s) {
								...
								$('.form-actions .btn-success', form).attr('disabled', false).find('i').removeClass('fa fa-sync fa-spin');
								UIToastr.init(s.type, 'Claim Update', s.message);
							},
							error: function () {
								// NProgress.done(true);
	...
	var initTable = function (location, fk_accounts) {
		...
		var t_options = '<option value=""><option value="">&nbsp;</option><option value="NON_FBF">NON FBF</option><option value="FBF_LITE">FBF LITE</option>'
		var options = '<option value="">&nbsp;</option>';
		for (i = 0; i < fk_accounts.length; i++) {
			options += '<option value="' + fk_accounts[i].account_name + '">' + fk_accounts[i].account_name + '</option>';
		}
		...
		var sDome = '<"row" <"col-md-4 col-sm-12" l><"col-md-8 col-sm-12 dataTables_length" <"filters" <"#order_type.dt_filter"> <"#account_name.dt_filter"> > f>>rt<"table-scrollable" <"col-md-5 col-sm-12" i><"col-md-7 col-sm-12" p>>';
		...
		table.dataTable({
			...
			"pageLength": 20,
			"language": {
				"lengthMenu": "  _MENU_ records per page | <span class='selected_checkbox_count'>0</span> selected",
			},
			"bDestroy": true,
			"order": [[2, "asc"]],
			"bSort": false,
		...
		$('#order_type').append("<label>Type: <select class='form-control input-inline'>" + t_options + "</select></label>");
		$("#order_type select").addClass('input-small');

		$('#account_name').append("<label>Account: <select class='form-control input-inline'>" + options + "</select></label>");
		$("#account_name select").addClass('input-small');

		$('#order_type select').bind('change', function () {
			console.log('order_type change');
			var val = $(this).val();
			table.api().columns(4).search(val ? '^' + val + '$' : '', true, false).draw();
		});
		$('#account_name select').bind('change', function () {
			console.log('account_name change');
			var val = $(this).val();
			table.api().columns(3).search(val ? '^' + val + '$' : '', true, false).draw();
		});
	};
		...
		form1.validate({
			...
			submitHandler: function (form) {
				...
				$.ajax({
					url: "ajax_load.php?token=" + new Date().getTime(),
					type: 'POST',
					data: "action=order_import&account_id=" + account_id + "&order_item_ids=" + order_item_ids,
					success: function (s) {
						s = $.parseJSON(s);
...
function init_flagging() {
	$(".flag").bind('click', function (e) {
		...
		currentReq = $.ajax({
			...
			cache: false,
			type: 'POST',
			data: "action=set_flag&order_item_id=" + order_item_id + "&flag=" + !is_active,
			beforeSend: function () {
				if (currentRequest != null) {
					currentRequest.abort();
				} else {
	...
	return {
		...
		init: function (spf_files, account_id) {
			...
			var myDropzone4 = new Dropzone(id, { // Make the whole body a dropzone
				url: "ajax_load.php?action=spf_generate&type=upload_image_attachements&account_id=" + account_id, // Set the url for your upload script location
			...
			myDropzone4.on("addedfile", function (file) {
				...
				$(".form-actions .btn-success").attr('disabled', true);
				var file_data = { "name": file.name, "size": file.size, "type": null, "fileUploadUrl": null, "path": null, "fileName": file.name };
			...
			myDropzone4.on("queuecomplete", function (progress) {
				// if (!error){
				$(id + " .dropzone-upload").attr('disabled', false).css('display', 'none').find('i').removeClass('fa fa-sync fa-spin');
				$(".form-actions .btn-success").attr('disabled', false);
				// }
			});
			...
			myDropzone4.on('error', function (file, response) {
				...
				response = $.parseJSON(response);
				$(file.previewElement).find('.dropzone-error').text('Error: ' + response.message);
				setTimeout(function () {
					$(id + " .progress-bar, " + id + " .progress, " + id + " .dropzone-start").css('opacity', '1');
				}, 350);
				return false;
			});
		}
